...
this.directions_ts = Date.now() - 10000;
    ...
    this.websocket_onopen = this.websocket_onopen.bind(this);
    this.websocket_onclose = this.websocket_onclose.bind(this);
    this.websocket_onerror = this.websocket_onerror.bind(this);
  ...
  console.log(`new WebSocket`)
    ...
    //this.ws.onclose = this.websocket_onclose.bind(this);
    /*function(e) {
      console.log('Socket is closed. Reconnect will be attempted in 1 second.', e.reason);
      setTimeout(function() {
        if (this.ws === undefined || this.ws.connect === undefined)
        {
          console.log(`this.ws.onclose: this.ws = new WebSocket`);
          this.ws = new WebSocket('ws://185.241.194.113:80');
        }
        else
        {
          console.log(`this.ws.onclose: this.ws.connect`);
          this.ws.connect();
        }
      }, 1000);
    }*/

    //this.ws.onerror = this.websocket_onerror.bind(this);
    /*(e) => {
      console.log('Socket encountered error: ', e.message, 'Closing socket');
      this.ws.close();

      console.log('Socket got an error. Reconnect will be attempted in 1 second. Reason:', e.reason);
      setTimeout(function() {
        if (this.ws === undefined || this.ws.connect === undefined)
          this.ws = new WebSocket('ws://185.241.194.113:80');
        else
...
API_registerDriver()
{
  ...
  if (this.API_driver_id === undefined)
  {
    ...
    websocket_onopen()
    {
        console.log(`this.websocket_onopen`)

        // If the driver is not created yet then create them
        if (this.API_driver_id === undefined)
        {
          if (this.API_registerDriver === undefined)
            console.log(`this.websocket_onopen: this.API_registerDriver === undefined`)
          else
            this.API_registerDriver();
        }

       console.log(`this.websocket_onopen: this.API_driver_id=${this.API_driver_id}`)
    }

   websocket_onclose(e)
   {
      console.log('websocket_onclose: Socket is closed. Reconnect will be attempted in 1 second. Reason:', e.reason);
      setTimeout(function()
      {
        if (this.ws === undefined || this.ws.connect === undefined)
        {
          console.log(`websocket_onclose: this.ws = new WebSocket`);
          this.ws = new WebSocket('ws://185.241.194.113:80');
          this.ws.onopen = this.websocket_onopen.bind(this);
          this.ws.onclose = this.websocket_onclose.bind(this);
          this.ws.onerror = this.websocket_onerror.bind(this);
        }
        else
    ...
    websocket_onerror(e)
    {
      console.log('websocket_onerror: Socket encountered error: ', e.message, 'Closing socket');
      this.ws.close();

      console.log('websocket_onerror: Socket got an error. Reconnect will be attempted in 1 second. Reason:', e.reason);
      setTimeout(function() {
        if (this.ws === undefined || this.ws.connect === undefined)
        {
          console.log(`websocket_onerror: this.ws = new WebSocket`);
          this.ws = new WebSocket('ws://185.241.194.113:80');
          this.ws.onopen = this.websocket_onopen.bind(this);
          this.ws.onclose = this.websocket_onclose.bind(this);
          this.ws.onerror = this.websocket_onerror.bind(this);
...
//  console.log(`API_on_message, this.API_driver_id: ${this.API_driver_id}, e:${JSON.stringify(e)}`)
  ...
  //console.log(`API_on_message, driver: ${JSON.stringify(this.driver)}`)
                ...
                // Change driver's coordinate smoothly
                //console.log(`call route_cache.update_object_coordinate(${this.API_driver_id}, ${JSON.stringify([{latitude: this.driver.lat, longitude: this.driver.lon}])})`)

                route_cache.update_object_coordinate(this.API_driver_id,
                  [{latitude: this.driver.lat, longitude: this.driver.lon}],
                  5000,
                  function(coord_array)
                  {
                    ...
                    // For some reason animateCamera does not work here. TODO
                   /* if (
                      this != null && this.driver !== {} && this.driver !== undefined && this.driver.lat !== undefined && this.driver.lon !== undefined && rotation !== undefined &&


                      this.map !== undefined && this.map !== null && this.map.animateCamera !== undefined)
                    {
                      var camera_coordinate = route_cache.getPointTowardsTo(
                        new_marker_coordinate,
                      ...
                      this.map.animateCamera({heading :
                        ...
                        pitch: 45, altitude: 300, zoom: 17}, {duration: 50});
                    }*/
