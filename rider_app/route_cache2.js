...
//      console.log(`get_route(${JSON.stringify(from)}, ${JSON.stringify(to)}): new_route_object=${JSON.stringify(new_route_object)}`);
  ...
  console.log(`get_route(${JSON.stringify(from)}, ${JSON.stringify(to)}): cache miss(${cache_misses}),  
  ${cache_hits1}(${two_point_cache.size}), ${cache_hits2}(${single_point_cache.size}), min_distance=${min_distance}, nearest_point_n=${nearest_point_n} cache_miss_gone_off_route=${cache_miss_gone_off_route}, url=${url}`);
    ...
    interval)
{
  ...
  else
  {
    ...
    // Calc average T for recent 10 points
    let sumT = T;
    for (let t of track.T_history)
      sumT += t;
    let avgT = sumT / (track.speed_history.length + 1);
  
    // If speed is less than 10sm per second then it's very likely a duplicated
    // coordinate - ignore it
    // If T is too small then ignore it either because it can influence V very bad
    if (V < 0.0001 || T < avgT/10)
    {
    ...
    for (let v of track.speed_history)
      sumV += v;
    let avgV = sumV / (track.speed_history.length + 1);

    // Calc mean squared deviation
    let msdV = (V-avgV) * (V-avgV);
    for (let v of track.speed_history)
      msdV += (v-avgV) * (v-avgV);
    msdV = Math.sqrt(msdV) / (track.speed_history.length + 1);

    // Calc average speed excluding bad points (too fast or too slow)
    /*let smoothAvgV = Math.abs(V - avgV)/msdV < 1 ? V : 0;
    let n = smoothAvgV == 0 ? 0 : 1;
    for (let v of track.speed_history)
    {
      if (Math.abs(v - avgV)/msdV < 1)
      {
        smoothAvgV += v;
        ++n;
      }
    ...
    // Find the maximum deviating v and exclude it from average
    let smoothAvgV;
    let maxDeviation = Math.abs(V - avgV);
    let maxV_Deviation = V;
    for (let v of track.speed_history)
    {
      if (Math.abs(v - avgV) > maxDeviation)
      {
        maxDeviation = Math.abs(v - avgV);
        maxV_Deviation = v;
      }
    }
    if (track.speed_history.length)
      smoothAvgV = (sumV - maxV_Deviation)/track.speed_history.length;
    else
      smoothAvgV = avgV;


      //console.log(`SPEED breakdown: S=${S}, T=${T}, V=${V}, avgV=${avgV}, msdV=${msdV}, smoothAvgV=${smoothAvgV}, maxV_Deviation=${maxV_Deviation}, maxDeviation=${maxDeviation}, track.speed_history.length=${track.speed_history.length}, track.T_history.length=${track.T_history.length}`);

      if (track.speed_history.length > 10)
        track.speed_history.shift();

      if (track.T_history.length > 10)
        track.T_history.shift();

      // Update object info
      var new_track = {
        speed: avgV,//smoothAvgV,
        speed_history: [... track.speed_history, V],
        T_history: [... track.T_history, T],
...
// Returns object's predicted route
export function get_object_predicted_route(id)
{
  var track = object_id_to_track.get(id);
  if (track === undefined)
    return undefined;
  else
    return track.predicted_route;
}
