...
import { StyleSheet, Dimensions, FlatList, Text, View, TextInput, Button } from 'react-native';
...
directionsQueryInProgressCounter = [0, 0, 0];
directionsQueryInProgressTimers = [undefined, undefined, undefined];

counterOfGetDirectionCalls = [0, 0, 0];

getDirections(startLoc, destinationLoc, number)
{
    console.log(`getDirections2 before First: number of timers for ${number} is ${this.directionsQueryInProgressTimers[number] === undefined ? 'UNDEFINED' : this.directionsQueryInProgressTimers[number].length}`);

  // First: clear all the timers accosiated with "number" type of girections
  // Why? There'are obviously obsolete
  if (this.directionsQueryInProgressTimers[number] !== undefined)
  {
    ...
    console.log(`getDirections2 before Second: query counter for ${number} is ${this.directionsQueryInProgressCounter[number]}`);
  ...
  if (this.directionsQueryInProgressCounter[number] != 0)
    ...
    console.log(`Inside getDirections2 this.counterOfGetDirectionCalls[${number}] == ${this.counterOfGetDirectionCalls[number]}`);
        ...
        fetch(`http://router.project-osrm.org/trip/v1/driving/${startLoc.longitude},${startLoc.latitude};${destinationLoc.longitude},${destinationLoc.latitude}?source=first&destination=last&roundtrip=false`)
          ...
          //console.log(`ENCODED: ${respJson.trips[0].geometry}`);
            ...
            if (number === 0)
              this.setState({
                rider_to_driver_route: coords,
                driver_to_A_distance : respJson.trips[0].distance,
                driver_to_A_duration : respJson.trips[0].duration
              })
            else
            if (number === 1)
              this.setState({
                A_to_B_route: coords,
                A_to_B_distance : respJson.trips[0].distance,
                A_to_B_duration : respJson.trips[0].duration
              })
            else
            if (number === 2)
              this.setState({
                A_to_best_price_route: coords
              })
...
// Store the best driver in a separate var from the state to have sync access to it
best_driver = {};
...
API_on_message(e)
{
  ...
  // Get the best driver
  this.best_driver = this._API_helper_get_driver_by_id(e, rider.best_driver_id);
  ...
  this.setState({
        ...
        bestDriver : {latitude: this.best_driver.lat, longitude: this.best_driver.lon},
       /* rider_to_driver_route: [
          {latitude: this.latA, longitude: this.lonA},
          {latitude: this.best_driver.lat, longitude: this.best_driver.lon}
        ],*/
       /* A_to_B_route: [
          {latitude: this.latA, longitude: this.lonA},
  ...
  // When it's 600 then it's SLOW. Set some limit for now. TODO: fix in a future
  if (e.data.drivers.length <= 10)
  {
   this.setState ({ driver_markers: [] })
    for (let driver of e.data.drivers)
    {
      if (driver.driver_id != this.best_driver.driver_id)
...
//directionsQueryStartTime = Date.now() - 10000;


buildDirections()
{
  console.log(`buildDirections`)//: directionsQueryInProgressCounter=${this.directionsQueryInProgressCounter}`);


  // If previous direction query has started less than 1 second ago and has
  // not finished then yet then don't issuea a new request
  //if (this.directionsQueryStartTime > Date.now() - 1000 && this.directionsQueryInProgressCounter)
    //  return;

  // Directions query is started
  // Note: directionsQueryInProgressCounter will be +1 in getDirections(, 3) on start
  // and -1 on finish - success or error
  //this.directionsQueryStartTime = Date.now();

  // Save directions from the best driver to the rider in this.state.rider_to_driver_route (
  // parameter 1 to this function) and A_to_B_route (param 2) and A_to_best_price_route (param 3)
  // Note: we need to do it here because this is once-a-second area, otherwise it's too frequentely
  // Note: directions are FROM DRIVER TO RIDER. Why? Because a driver will come over to a rider, not
  //  vice versa :-) And if a rider wants to a driver to come over faster then she/he needs to
  //  go onto this route, because this is how the driver will pick up her/his faster
  this.getDirections(
    {latitude: this.best_driver.lat, longitude: this.best_driver.lon},
    {latitude: this.latA, longitude: this.lonA},
    0)
  ...
  // Walk to the best price point if you want even cheaper
  if (this.state.bestPrice_coordinate != undefined)
    this.getDirections(
      {latitude: this.latA, longitude: this.lonA},
      {latitude: this.state.bestPrice_coordinate.latitude, longitude: this.state.bestPrice_coordinate.longitude},
...
<MapView.Polyline
      ...
      if (this.state.is_order_placed)
      {
       ...
       this.render_price_and_distance = (
        ...
        Подача {((this.state.driver_to_A_duration)/60.0).toFixed(1)} минут
        , {((this.state.driver_to_A_distance)/1000.0).toFixed(1)} км
        
        {"\n"}
        
        Поездка {((this.state.A_to_B_duration)/60.0).toFixed(1)} минут
        , {((this.state.A_to_B_distance)/1000.0).toFixed(1)} км
...
{this.state.driver_markers.map((marker) => (
    <MapView.Marker
