...
//  console.log(`API_on_message, this.API_driver_id: ${this.API_driver_id}, e:${JSON.stringify(e)}`)
      ...
      this.assigned_order_id != -1)
  {
    ...
    if (this.state.assigned_order_status == "at_A")
    {
      ...
      let wait_time_so_far = Math.trunc(Date.now()/1000) - this.best_order.ts_driver_at_A;
        ...
        <View style={styles.container}>
          <ScrollView style={{flex: 1, alignItems: 'center', justifyContent: 'center', position: 'absolute', left : 10, right : 10, top: 40}}>
            <View style={{flex: 1, flexDirection: 'row'}}>
              <Text style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}> INSTANT </Text><MaterialCommunityIcons name="speedometer" size={24} color="black"/><Text style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}> TAXI</Text>
            </View>
        ...
        if (this.state.at_A_is_toll_waiting)
        {
          render_assigned_order_info = (
            <View style={{flex: 1, position: 'absolute', left : 10, right : 10, alignItems: "center"}}>
              ...
              <View style={{flex: 1}}>
                <Text style={{fontFamily: 'roboto', textTransform: 'uppercase', fontWeight: 'bold', fontSize: '15'}}>Платное ожидание {this.state.wait_time_toll_minutes} мин, {this.state.wait_time_toll_seconds} сек</Text>
                <View style={{flex: 1, flexDirection: 'row'}}>
                  <AntDesign onPress={() => {this.API_driver_accepted_toll_wait(this.state.at_A_waiting_toll_seconds)}} name="checksquareo" size={36} color="black"/>
                  <MaterialIcons onPress={() => {this.API_driver_accepted_toll_wait(0)}} name="cancel" size={36} color="black"/>
                </View>
              </View>
            </View>
          )
...
*/
      ...
      if (this.state.price_for_toll_wait && this.state.price_for_toll_wait != -1)
        price = `${Math.round(this.state.best_order_price)} (+ ${this.state.price_for_toll_wait})`;
      else
	...
	strokeWidth={10}/>
        ...
        <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 30}}>
          ...
          <View style={{flex: 5, flexDirection: 'row'}}>
            ...
            <View style={{flex: 1}}><Text></Text></View>
            <Text style={{flex: 1, color: 'black', fontSize: '19px', fontWeight: 'bold'}}>{price}</Text>
            <MaterialCommunityIcons style={{flex: 1}} name="currency-rub" size={24} color="black" />
          </View>
        ...
        <View style={{flex: 1, position: 'absolute', left : 10, right : 10, bottom: 20, alignItems: "center"}}>
          ...
          <View style={{flex: 0.4, flexDirection: 'row'}}>
            <TouchableOpacity onPress={()=>{this.setState({is_in_chat: 1})}}><MaterialIcons name="feedback" size={36} color="black"/>
</TouchableOpacity>
            <TouchableOpacity onPress={()=>{this.setState({is_in_chat: 1})}}><MaterialIcons name="chat" size={36} color="black"/>
</TouchableOpacity>
            ...
            <TouchableOpacity onPress={()=>{this.setState({is_in_chat: 1})}}><MaterialIcons name="watch-later" size={36} color="black"/></TouchableOpacity>
