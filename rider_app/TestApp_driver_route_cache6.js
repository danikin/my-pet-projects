...
// point            - its coordinate
// freshness_time   - the time in miliseconds during which the coordinate
...
// predicted_route  - a route that was predicted by a caller and that will
//  be used to understand the direction where to go. The object will be moving
//  along this route with the speed taken from it recent two coordinate updates.
//
...
//  speed taken by two recent coordinates (either from the predicted route or from
//  the update whichever comes first)
//
...
//
//  Note: predicted_route can be empty. In that case the function acts like it's finished
//  Note: can comprise only one coordinate - it's up to a caller to supply a
...
//    the next predicted coordinate. So a caller MAY NOT pass a predicted route
//    that has some already gone part - it's only a predicted part
    ...
    interval)
{
  ...
  if (track === undefined)
    track = {
      ...
      recent_index: 0,
      freshness_time: Date.now() + freshness
    };
  else
  {
    // Update object info
    var new_track = {
      speed: track.speed === undefined ?
        getDistancePointToPoint(track.predicted_route[0],
                                predicted_route[0]) /
                                        (Date.now() - track.recent_time) :
        track.seed,
  ...
  setTimeout((callback, interval) => {notification_processor(callback, interval)}, interval);
...
function notification_processor(callback, interval)
{
  ...
  var result = [];
    ...
    for (let [object_id, object] of object_id_to_track)
    {
      ...
      // If the freshness time has expired then don't update
      if (object.freshness_time < now)
        continue;

      // Iterate all route trying to find where we are "now"
      while (object.recent_index < object.predicted_route.length-1)
      {
        ...
        var ratio = (now - object.recent_time) * object.speed / full_distance;
       ...
       //     console.log(`route_cache.start_notification for object ${object_id} ASSERT: object.predicted_route.length=${object.predicted_route.length}, object.recent_callback_pos=${object.recent_callback_pos}, object.recent_callback_time=${object.recent_callback_time}, ind=${ind}, object.predicted_route.length=${object.predicted_route.length}, timer=${timer}, point=${point}`);*/
