...
//  console.log(`API_on_message, this.API_driver_id: ${this.API_driver_id}, e:${JSON.stringify(e)}`)
      ...
      this.assigned_order_id != -1)
  {
    ...
    console.log(`this.best_order.price_for_toll_wait=${this.best_order.price_for_toll_wait}, this.state.page=${this.state.page}, this.state.assigned_order_status=${this.state.assigned_order_status}`);
      ...
      let render_onboarding_bottom = (
          ...
          onPress={() => this.setState({page: "best_order"})}>
            ...
            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
              ...
              <TouchableOpacity onPress={() => {this.setState({page: "onboarding_stat"})}}><Ionicons name="md-stats" size={36} color="black" /></TouchableOpacity>
            </View>
          </View>
        )
            ...
            <View style={{flex: 10, justifyContent: 'left'}}>
              ...
              <View style={{flex: 10, borderTopWidth: 1, alignItems: 'stretch', justifyContent: 'left'}}>
                ...
                <Table borderStyle={{borderWidth: 0.5, marginTop: 10, marginBottom: 10}}>
                  <Row data={ride_head} style={{ backgroundColor: '#DDDDDD'}} />
                    {ride_data.map((entry, index) => (
              ...
              <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
                <TouchableOpacity><FontAwesome5 name="car-side" size={48} color="black" /></TouchableOpacity>
                <TouchableOpacity><MaterialCommunityIcons name="bank-transfer-in" size={24} color="black" /></TouchableOpacity>
                <TouchableOpacity><MaterialCommunityIcons name="bank-transfer-out" size={24} color="black" /></TouchableOpacity>
                <TouchableOpacity><MaterialCommunityIcons name="cash-refund" size={24} color="black" /></TouchableOpacity>
                <TouchableOpacity><AntDesign name="gift" size={24} color="black" /></TouchableOpacity>
                <TouchableOpacity><MaterialIcons name="account-balance-wallet" size={24} color="black" /></TouchableOpacity>
              </View>
      ...
      return (
        <View style={styles.container}>
          ...
          <ScrollView style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}>
            ...
            <View style={{flex: 2}}><Text></Text></View>
            <View style={{flex: 20, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'left'}}>
              <View><Text  style={{fontSize: '12px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>1. Пассажиры платят только наличными или через Сбербанк.Онлайн, вы сразу получаете деньги, а не ждете их неделю</Text></View>
              <View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '12px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>2. Низкая комиссия, всего 3% или смена 150 руб. Вы сначала получаете деньги от пассажира, а потом платите комиссию или за смену через Киви, Элекснет или Сбербанк.Олайн.</Text></View>
              <View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '12px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>3. Вы всегда видите самый выгодный заказ, с автоматическим учетом в цене долгой подачи и возврата в место высокого спроса</Text></View><View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '12px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>4. Вы всегда видите цену, откуда, куда</Text></View><View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '12px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>5. Никаких штрафов за отказ, роботов, обязаловок</Text></View><View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '12px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>6. Свой тариф, фильтр районов, заказы по пути, фильтр по дистанции</Text></View>
            </View>
          </ScrollView>
          </View>
          ...
          render_assigned_order_info = (
            <View style={{flex: 1, position: 'absolute', left : 10, right : 10, alignItems: "center"}}>
              ...
              <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
                ...
                <View style={{flex: 1, flexDirection: 'row'}}>
                  <TouchableOpacity onPress={() => {this.API_driver_accepted_toll_wait(this.state.at_A_waiting_toll_seconds)}}><AntDesign name="checksquareo" size={36} color="black"/></TouchableOpacity>
                  <TouchableOpacity onPress={() => {this.API_driver_accepted_toll_wait(0)}}><MaterialIcons name="cancel" size={36} color="black"/></TouchableOpacity>
                </View>
              </View>
            </View>
          )
