...
//  console.log(`API_on_message, this.API_driver_id: ${this.API_driver_id}, e:${JSON.stringify(e)}`)
  ...
  {
    console.log(`API_on_message(for driver ${this.API_driver_id}): got a wrong driver from the message (probably the driver has not been registred yet): this.driver.driver_id=${this.driver.driver_id}`);
    return;
  }
      ...
      this.state.best_order_id == 0)
  {
    if (this.state.page == "best_order" || this.state.page == "splash")
    {
      ...
      console.log(`API_on_message(for driver ${this.API_driver_id}): no best order: this.driver.best_rider_id=${this.driver.best_rider_id}, this.best_order.unassigned_order_id=${this.best_order.unassigned_order_id}, this.state.best_order_id=${this.state.best_order_id}, this.assigned_order_id=${this.assigned_order_id}`);
    }
  }
...
best_order.latB + "&lon=" + best_order.lonB + "&resolution=11&ring_size=1")
  ...
  this.setState({
    ...
    if (is_no_best_order)
      console.log(`API_on_message(for driver ${this.API_driver_id}): after setState, this.state.best_order_id=${this.state.best_order_id}`)
      ...
      */
      ...
      // On the way to A
      {
          ...
          //if (this.state.distance1 < 200)
            render_assigned_order_info = (<TouchableOpacity style={{flex: 1, position: 'absolute', left : 10, right : 10, alignItems: "center", backgroundColor: "#d1ffd2", padding: 10, borderColor: 'black', borderWidth: 1}} onPress={() => { return this.setState({assigned_order_status: "at_A", next_turn_description_line1: "Пассажир где-то рядом :-)", next_turn_description_line2: ""})}}>
...
coordinates={this.state.coords2}
	...
	strokeWidth={10}/>
        ...
        <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 30}}>
          ...
          <View style={{flex: 1}}><Text></Text></View>
          <View style={{flex: 1}}>
            <Text style={{color: 'black', fontSize: '11px', fontWeight: 'bold'}}>{(this.state.page == "ride_begins")? this.state.addr_B : this.state.addr_A}</Text>
          </View>
          <View style={{flex: 1}}><Text></Text></View>
