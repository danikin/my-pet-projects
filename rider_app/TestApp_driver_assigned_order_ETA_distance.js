...
// 2. Rebuilds the route if a driver is gone offroute (TODO!)
// 3. Determines remaining time and distance (TODO!)
optimizeRoute(route)
{
  ...
  // Update distance information (distance to A)
...
{  
 ...
 //     page: "assigned_order",
      ...
      coords1 : this.optimizeRoute(this.state.coords1)
    })
...
// style={{ position: 'absolute', left : 10, right : 10, bottom: 80, height: 40}}
    ...
    {
        // Here should be a navigtor the to pickup point
        ...
        return (
          ...
          <View style={{flex: 1, position: 'absolute', bottom: 10, height: 30}}>
            <Text>{(((Date.now() - this.order_acceptance_ts) / 1000 + this.state.duration1)/ 60).toFixed(0)} мин</Text>
            <Text>{(this.state.distance1/1000).toFixed(1)} км</Text>
          </View>
