...
optimizeRoute(route)
{
  ...
  // If there are no good turns then unset the callout
  if (!is_found)
    this.setState({
    next_turn_coordinate : undefined,
    next_turn_distance : undefined,
    next_turn_description : undefined
    });
...
buildDirections()
{
  ...
  route_cache.get_route({latitude: this.driver.lat, longitude: this.driver.lon}, {latitude: this.best_order.latA, longitude: this.best_order.lonA},
  ...
  route_cache.get_route({latitude: this.best_order.latA, longitude: this.best_order.lonA}, {latitude: this.best_order.latB, longitude: this.best_order.lonB},
  function(route_object)
  {
    this.setState({
...
// style={{ position: 'absolute', left : 10, right : 10, bottom: 80, height: 40}}
    ...
    if (this.state.page == "assigned_order")
    {
      ...
      */
      ...
      {
            render_assigned_order_info = (<TouchableOpacity style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 30, alignItems: "center", backgroundColor: "rgba(100,255,100,100)", padding: 10, borderColor: 'black', borderWidth: 1}} onPress={() => { return this.setState({assigned_order_status: "", page: "best_order"})}}>
      ...
      if (this.state.assigned_order_status == "ride_begins")
      {
        ...
        if (this.state.distance2 < 100)
          render_assigned_order_info = (<TouchableOpacity style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 30, alignItems: "center", backgroundColor: "rgba(100,255,100,100)", padding: 10, borderColor: 'black', borderWidth: 1}} onPress={() => { return this.setState({assigned_order_status: "payment"})}}>
      ...
      return (
          ...
          >
            ...
            <MapView.Marker coordinate={this.state.next_turn_coordinate} ref={ref => { this.marker = ref}}>
              <View></View>
              <MapView.Callout>
                <View style={{flex: 1, backgroundColor: '#4ba9c8'}}>
                <Text style={{color: 'white', fontFamily: 'roboto', fontWeight:'bold', fontSize: '20'}}>Через {Math.round(this.state.next_turn_distance)} метров{"\n"}{this.state.next_turn_description}</Text>
                </View>
              </MapView.Callout>
            </MapView.Marker>
