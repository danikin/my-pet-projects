...
export function get_route(from, to, on_route_ready)
{
  ...
  if (from === undefined || from.latitude === undefined || from.longitude === undefined
    || from === {} ||
...
//      console.log(`get_route(${JSON.stringify(from)}, ${JSON.stringify(to)}): new_route_object=${JSON.stringify(new_route_object)}`);
      ...
      if (distance_to_first_section < 30)
      {
        // Different in distances because of substitution the first point with "from"
        var distance_change = getDistancePointToPoint(from, new_route_object.route[1]) -             
            getDistancePointToPoint(new_route_object.route[0], new_route_object.route[1]);

        var decorated_new_route_object = new_route_object;

        var duration_change = (decorated_new_route_object.distance) ? (distance_change *
      ...
      else
      {
        ...
        console.log(`get_route(${JSON.stringify(from)}, ${JSON.stringify(to)}),
          ...
          ${JSON.stringify(new_route_object.route[0])},
          ${JSON.stringify(new_route_object.route[1])}`);
