...
export /*default*/ class DriverApp extends React.Component {
  ...
  componentDidMount() {
    ...
    this.on_driver_cooridinate_change = this.on_driver_cooridinate_change.bind(this);
  
    // Update driver position/register a driver
    // It's done only once, altough can be done more
    // TODO! This is a broadcast event, so we don't need to send it if a driver is
    //  already assigned. For now it's ok because we create a new driver from scratch
    //  on each app start, but in a future we MUST take this into account
    server_api_sdk.api_driver_set_position(this.state.driver_region,
      this.on_message_received,
      this.on_driver_cooridinate_change);
  }
...
buildDirections_best_order(driver_current_coordinate, order)
{
  //console.log(`buildDirections_best_order(${JSON.stringify(driver_current_coordinate)}, ${JSON.stringify(order)})`);

  // Build the route from driver's position to A
  route_cache.get_route(
      driver_current_coordinate,
...
buildDirections_navigator(is_route_to_B, driver_current_coordinate, order)
{
  // Navigator is for the driver going either to A or to B
  var to_coordinate = is_route_to_B ?
    {latitude: order.latB, longitude: order.lonB} :
    {latitude: order.latA, longitude: order.lonA};

  // Build the route
  route_cache.get_route(driver_current_coordinate, to_coordinate,
    function(route_object)
    {
      ...
      var decoration = this.decorateRoute(driver_current_coordinate, route_object.route);
...
// This is a handle for changing driver's coordinate
// Note: as of now this handler is called in websockets by processing coordinate
//  changes from the simulator. In a future this will be notified via GPS
on_driver_cooridinate_change(driver_current_coordinate)
{
  /*
  *   Note: this method can be called VERY FREQUENTLY! So everything inside MUST
  *     work VERY FAST
  */

  // This is to save the previous driver coordinate which is used for
  // determining the rotation
  this.prev_driver_coordinate_array.push(driver_current_coordinate);
  ...
  //  Note: we use the coordinate from the server because for test reasons motion is
  ...
  route_cache.update_object_coordinate(
    server_api_sdk.api_get_current_driver().driver_id,
    [driver_current_coordinate],
    5000,
        ...
        this.map.setCamera !== undefined)
  {
    ...
    this.map.setCamera({heading : 0,
      center: driver_current_coordinate,
      pitch: 45, altitude: 300, zoom: 17}, {duration: 1100});
  }

  let order = server_api_sdk.api_get_current_order();

  // 3. If there is a best order and no assigned order then build directions
  // Note: we do it here to handle changes in driver's coordinate
  if (order !== undefined && order.status == "order")
    this.buildDirections_best_order(driver_current_coordinate, order);

  // 4. If the driver has an assigned order then animate camera and build the route
  //    for the navigator
  if (server_api_sdk.api_get_current_order() !== undefined &&
        server_api_sdk.api_get_current_order().status == "assigned_order")
  {
    let order = server_api_sdk.api_get_current_order();
    ...
    if (distance >= 1)
    {
      let rotation = route_cache.getGoodRotation(
          driver_previous_coordinate, driver_current_coordinate) + 90;

      // Set driver's coordinate on map
      this.setState({
        driver_region : {
          latitude: driver_current_coordinate.latitude,
          longitude: driver_current_coordinate.longitude,
          latitudeDelta: this.state.driver_region.latitudeDelta,
          longitudeDelta: this.state.driver_region.longitudeDelta}
          //best_order: order
      })
...
} // on_driver_cooridinate_change

on_message_received()
{
  //console.log(`on_message_received2: driver=${JSON.stringify(server_api_sdk.api_get_current_driver())}, order=${JSON.stringify(server_api_sdk.api_get_current_order())}`);

  /*
  *   Note: this method is not very frequent - it's about changes in statuses, orders etc
  *   Note: we MUST NOT use driver.lat and driver.lon because they can be outdated.
  *     Fresh driver coordinate is received via on_driver_cooridinate_change (either by
  *     GPS or simulator via a personal message to the websocket)
  */

  let driver_current_coordinate =
    this.prev_driver_coordinate_array[this.prev_driver_coordinate_array.length-1];

  // If driver's coordinate is unknown then take it from the server response
  // Note: this is a rare situation (not in the test) if GPS is working
  if (this.prev_driver_coordinate_array.length == 0 ||
        driver_current_coordinate === undefined)
  {
    let driver = server_api_sdk.api_get_current_driver();
    driver_current_coordinate = {latitude: driver.lat, longitude: driver.lon};
    console.log(`on_message_received2: driver_current_coordinate is unknown - so take it from the server response: driver_current_coordinate=${JSON.stringify(driver_current_coordinate)}`);
  }
  ...
  // 1. In any state we need the updated info about the best order if it exists
  if (order !== undefined)
  {
    this.setState({
    ...
    localtion_cache.resolve_coordinate_to_address({latitude: order.latA, longitude: order.lonA},
    function (address)
      {
        this.setState({addr_A: address});
      }.bind(this));
    localtion_cache.resolve_coordinate_to_address({latitude: order.latB, longitude: order.longB},
      function (address)
      {
        this.setState({addr_B: address});
      }.bind(this));
  } // if (order !== undefined)

  // 2. If the app is still on splash or best_order, but the driver is assigned then
  ...
  // 3. If there is a best order and no assigned order then build directions to show on the map
  // Note: we do it here to handle changes in the best order
  if (order !== undefined && order.status == "order")
    this.buildDirections_best_order(driver_current_coordinate, order);
...
get_stat(stat_type)
{
  server_api_sdk.api_driver_get_stat(stat_type, 0, 2000000000, function(entries)
  {
    console.log(`get_stat: entries=${JSON.stringify(entries)}`);

    // This is for Table component to work properly
    if (entries === undefined)
      entries = [[]];

    this.setState({
      page: "onboarding_stat",
      stat_table_type: stat_type,
...
render()
{
    ...
    */
    ...
    if (this.state.page.startsWith("onboarding") || this.state.page == "splash")
    {
      ...
      let render_onboarding_bottom = (
          ...
          onPress={() => this.setState({page: "best_order"})}>
            ...
            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
              ...
              <TouchableOpacity><MaterialIcons name="ondemand-video" size={36} color="black"/></TouchableOpacity>
              ...
              <TouchableOpacity onPress={() => {this.setState({page: "onboarding_geo_settings"})}}><SimpleLineIcons name="location-pin" size={36} color="black" /></TouchableOpacity>
              <TouchableOpacity onPress={() => {this.setState({page: "onboarding_settings"})}}><Feather name="settings" size={36} color="black" /></TouchableOpacity>
              <TouchableOpacity onPress={() => {this.get_stat("rides")}}><Ionicons name="md-stats" size={36} color="black" /></TouchableOpacity>
              <TouchableOpacity onPress={() => {this.setState({page: "onboarding_documents"})}}><Entypo name="documents" size={36} color="black" /></TouchableOpacity>
            </View>
          </View>
        )
      ...
      if (this.state.page == "onboarding_stat")
      {
        ...
        */
        ...
        const stat_descriptions = {
          "rides_description" : "Все заказы (включая отмены)",
          "aggr_rides_description" : "Агрегация по заказам (включая отмены)",
          "shift_purchases_description" : "Покупки смен",
          "promos_description" : "Промо акции",
          "payments_description" : "Пополнения баланса",
          "withdrawals_description" : "Все выводы денег",
          "cashbacks_description" : "Все кэшбэки",
          "bonuses_description" : "Поступления бонусов",
        ...
        */
        ...
        "cashbacks_head" : [
          'Дата',
          'Сумма',
          'За что', // 10 rub/day for online app
        ],
        ...
        "bonuses_head" : [
          'Дата',
          'Сумма',
          'За что', // 4500 rub one-time bonus
        ],
          ...
          <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 40, bottom: 150}}>
            ...
            <View style={{flex: 10, justifyContent: 'flex-start'}}>
              ...
              <View style={{flex: 7.8, alignItems: 'stretch', justifyContent: 'flex-start'}}>
                ...
                <Table borderStyle={{borderWidth: 0.5, marginTop: 10, marginBottom: 10}}>
                  <Row data={stat_heads[this.state.stat_table_type + "_head"]} style={{ backgroundColor: '#DDDDDD'}} />
                    {/*(*/this.state.stat_entries/*stat_data[this.state.stat_table_type + "_data"])*/.map((entry, index) => (
              ...
              <View style={{flex: 1.5, borderBottomWidth: 0.5, flexDirection: 'row', justifyContent: 'space-between'}}>
                <TouchableOpacity onPress={()=>{this.get_stat("rides")}}><MaterialCommunityIcons name="car" size={36} color="black" /></TouchableOpacity>
                <TouchableOpacity onPress={()=>{this.get_stat("aggr_rides")}}><MaterialCommunityIcons name="car-multiple" size={36} color="black" /></TouchableOpacity>
                <TouchableOpacity onPress={()=>{this.get_stat("shift_purchases")}}><FontAwesome5 name="hand-holding-usd" size={36} color="black" /></TouchableOpacity>
                
                <TouchableOpacity onPress={()=>{this.get_stat("promos")}}><MaterialIcons name="present-to-all" size={36} color="black" /></TouchableOpacity>
              </View>
              <View style={{flex: 1.5, borderBottomWidth: 0.5, flexDirection: 'row', justifyContent: 'space-between'}}>
                <TouchableOpacity onPress={()=>{this.get_stat("payments")}}><MaterialCommunityIcons name="bank-transfer-in" size={42} color="black" /></TouchableOpacity>
                <TouchableOpacity onPress={()=>{this.get_stat("withdrawals")}}><MaterialCommunityIcons name="bank-transfer-out" size={42} color="black" /></TouchableOpacity>
                <TouchableOpacity onPress={()=>{this.get_stat("cashbacks")}}><MaterialCommunityIcons name="cash-refund" size={36} color="black" /></TouchableOpacity>
                <TouchableOpacity onPress={()=>{this.get_stat("bonuses")}}><AntDesign name="gift" size={36} color="black" /></TouchableOpacity>
                <TouchableOpacity onPress={()=>{this.get_stat("balance")}}><MaterialIcons name="account-balance-wallet" size={36} color="black" /></TouchableOpacity>
              </View>
      ...
      if (this.state.page == "onboarding_settings")
      {
          return (
        <View style={styles.container}>
          ...
          <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 40, bottom: 150}}>
            ...
            <View style={{flex: 1.6, justifyContent: 'center'}}>
              <View style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}>
                ...
                <View>
                  <Text style={{fontSize: 17, color: 'black', fontWeight: 'bold', fontFamily: 'roboto'}}>TAXI - настройки</Text>
                </View>
              </View>
              <View style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}><Text style={{fontSize: 11, color: 'black', fontWeight: 'bold', fontFamily: 'roboto', fontStyle: 'italic'}}>driver</Text></View>
            </View>
            ...
            <View style={{flex: 10, justifyContent: 'flex-start'}}>
              ...
              <View style={{flex: 7.8, alignItems: 'stretch', justifyContent: 'flex-start'}}>
                ...
                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Уведомлять о новом заказе</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Уведомлять о повышении цены заказа</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Включить пуши</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Включить звуковые уведомления</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Принимать заказ автоматом</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Курящий салон</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Есть детское кресло</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Принимаю наличные</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Принимаю Сбербанк.Онлайн</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Не уведомлять до</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <TextInput style={{flex: 0.2, borderWidth: 1}} value="09:00"/>
                </View>

                <View style={{flex: 0.3}}><Text></Text></View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Не уведомлять после</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <TextInput style={{flex: 0.2, borderWidth: 1}} value="21:00"/>
                </View>
      ...
      if (this.state.page == "onboarding_geo_settings")
      {
          return (
        <View style={styles.container}>
          ...
          <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 40, bottom: 150}}>
            ...
            <View style={{flex: 1.6, justifyContent: 'center'}}>
              <View style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}>
                ...
                <View>
                  <Text style={{fontSize: 17, color: 'black', fontWeight: 'bold', fontFamily: 'roboto'}}>TAXI - гео-настройки</Text>
                </View>
              </View>
              <View style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}><Text style={{fontSize: 11, color: 'black', fontWeight: 'bold', fontFamily: 'roboto', fontStyle: 'italic'}}>driver</Text></View>
            </View>
            ...
            <View style={{flex: 10, justifyContent: 'flex-start'}}>
              ...
              <View style={{flex: 7.8, alignItems: 'stretch', justifyContent: 'flex-start'}}>
                ...
                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Максимальная дистанция подачи (км)</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <TextInput style={{flex: 0.2, borderWidth: 1}} value="2.5"/>
                </View>

                <View style={{flex: 0.2}}><Text></Text></View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Максимальная дистанция поездки (км)</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <TextInput style={{flex: 0.2, borderWidth: 1}} value="10"/>
                </View>

                <View style={{flex: 0.2}}><Text></Text></View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Заказы около адреса</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <TextInput style={{flex: 1, borderWidth: 1}} value="Улица Ленина 25"/>
                </View>
                ...
                <View style={{flex: 1, borderTopWidth: 1}}>
                  <View style={{flex: 0.3}}><Text></Text></View>
                  <Text style={{flex: 1, textTransform: 'uppercase', fontWeight: 'bold'}}>Не назначать заказы в эти районы</Text>
                </View>

               <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Трехозерка</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Краснозатонский</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Седкыркещ</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Верхний чов</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Верхняя максаковка</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>

                <View style={{flex: 1, flexDirection: 'row'}}>
                  <Text style={{flex: 1}}>Выльгорт</Text>
                  <View style={{flex: 0.1}}><Text></Text></View>
                  <FontAwesome5 style={{flex: 0.2}} name="check-square" size={24} color="black" />
                </View>
      ...
      if (this.state.page == "onboarding_documents")
      {
        return (
          ...
          <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 40, bottom: 150}}>
            ...
            <View style={{flex: 1.6, justifyContent: 'center'}}>
              <View style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}>
                ...
                <View>
                  <Text style={{fontSize: 17, color: 'black', fontWeight: 'bold', fontFamily: 'roboto'}}>TAXI - документы</Text>
                </View>
              </View>
              <View style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'row'}}><Text style={{fontSize: 11, color: 'black', fontWeight: 'bold', fontFamily: 'roboto', fontStyle: 'italic'}}>driver</Text></View>
            </View>
...
<View style={{flex: 0.8, flexDirection: 'row', justifyContent: 'flex-end'}}>
          <Text style={{borderColor: 'gray', backgroundColor: 'white', borderWidth: 1, fontSize: 15, fontWeight: 'bold', flex: 0.9}}>Ваш портрет</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 0, camera_type: Camera.Constants.Type.front})}}><AntDesign name={this.state.pic_status[0]} size={36} color="black"/></TouchableOpacity>
        </View>

        <View style={{flex: 0.8, flexDirection: 'row', justifyContent: 'center'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth: 1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>Главная страница паспорта</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 1, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[1]} size={36} color="black"/></TouchableOpacity>
        </View>       

<View style={{flex: 0.8, flexDirection: 'row', justifyContent: 'space-around'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth:1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>Прописка в паспорте</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 2, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[2]} size={36} color="black"/></TouchableOpacity>
        </View>

        <View style={{flex: 0.8, flexDirection: 'row', justifyContent: 'space-between'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth: 1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>Права</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 3, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[3]} size={36} color="black"/></TouchableOpacity>
        </View>

        <View style={{flex: 0.8, flexDirection: 'row', justifyContent: 'space-evenly'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth: 1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>Обратная сторона прав</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 4, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[4]} size={36} color="black"/></TouchableOpacity>
        </View>


        <View style={{flex: 0.8, flexDirection: 'row'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth: 1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>СТС</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 5, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[5]} size={36} color="black"/></TouchableOpacity>
        </View>

        <View style={{flex: 0.8, flexDirection: 'row'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth:1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>Обратная сторона СТС</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 6, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[6]} size={36} color="black"/></TouchableOpacity>
        </View>

        <View style={{flex: 0.8, flexDirection: 'row'}}>
          <Text style={{borderColor:'gray',backgroundColor:'white',borderWidth: 1,fontSize: 15,fontWeight: 'bold', flex: 0.9}}>Чистая машина с номером</Text>
          <TouchableOpacity style={{flex:0.2}} onPress={() => {this.setState({page: "camera", page_camera_n: 7, camera_type: Camera.Constants.Type.back})}}><AntDesign name={this.state.pic_status[7]}size={36} color="black"/></TouchableOpacity>
        </View>
...
<View style={{flex: 0.8, flexDirection: 'row', justifyContent: 'space-around'}}>
   ...
   // Camera page
    ...
    {
      console.log(`show_pic: this.state.photo=${this.state.photo}, this.state.page_camera_n=${this.state.page_camera_n}`);
              ...
              <View style={{flex:0.3, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}>
                ...
                <View style={{flex:0.5, flexDirection: 'column'}}>
                  <TouchableOpacity style={{flex:0.6}} onPress={() => {
                    ...
                    this.setState({page: "onboarding_documents"
                    })}}>
...
coordinates={this.state.coords2}
	...
	strokeWidth={10}/>
      ...
      <View style={{flex:1, position: 'absolute', bottom: 10}}>
        ...
        <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
          <TouchableOpacity onPress={()=>{this.setState({page: "onboarding_feedback"})}}><MaterialIcons name="feedback" size={36} color="black" /></TouchableOpacity>
          <TouchableOpacity onPress={() => {this.setState({page: "onboarding_QnA"})}}><Entypo name="help" size={36} color="black" /></TouchableOpacity>
          <TouchableOpacity onPress={() => {this.setState({page: "order_list"})}}><FontAwesome name="list-ol" size={36} color="black" /></TouchableOpacity>
          <TouchableOpacity onPress={() => {this.setState({page: "onboarding_geo_settings"})}}><SimpleLineIcons name="location-pin" size={36} color="black" /></TouchableOpacity>
          <TouchableOpacity onPress={() => {this.setState({page: "onboarding_settings"})}}><Feather name="settings" size={36} color="black" /></TouchableOpacity>
          <TouchableOpacity onPress={() => {this.get_stat("rides")}}><Ionicons name="md-stats" size={36} color="black" /></TouchableOpacity>
        </View>
    ...
    // The of 3 good orders (sorted by distance to A)
    if (this.state.page == "order_list")
    {
return (
      <View style={styles.container}>
        <MapView provider={PROVIDER_GOOGLE} style={styles.mapStyle}
...
<MapView.Marker coordinate={this.state.driver_region}>
  <View><MaterialIcons name="local-taxi" size={36} color="#4B470C"/></View>
      ...
      <View style={{flex:1, position: 'absolute', top: 30}}>
        ...
        <View style={{flex: 10, alignItems: "center"}}><Text style={{color: 'black', fontSize: 21, fontWeight: 'bold'}}>Список лучших заказов</Text></View>
        <View style={{flex: 5}}><Text></Text></View>
