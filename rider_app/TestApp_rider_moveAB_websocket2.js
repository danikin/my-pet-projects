...
API_moveA(latitude, longitude)
{
  ...
  this.ws.send(`{"event":"test_update_price_view_A", "data":{"rider_id":1804289383,"lat":${latitude},"lon":${longitude}}}`);
}
...
API_moveB(latitude, longitude)
{
  ...
  //console.log(latitude, longitude);
  this.ws.send(`{"event":"test_update_price_view_B", "data":{"rider_id":1804289383,"lat":${latitude},"lon":${longitude}}}`);

  // If B is specified and there is no rider then create it
  if (this.API_rider_id == undefined)
  {
    this.ws.send(`{"event":"test_price_view", "data":{"latA":${this.latA},"lonA":${this.lonA},"latB":${this.latB},"lonB":${this.lonB}}}`);
    this.API_rider_id = -1;
  }
  ...
  if (this.API_rider_id == -1)
  {
    ...
    for(let rider of e.data.riders)
    {
      console.log("rider.latA=" + rider.latA + ",rider.lonA=" + rider.lonA + ",rider.latB="
        ...
        var abs1 = Mathrider.latA - this.latA);
        var abs2 = Math.abs(rider.lonA - this.lonA);
        var abs3 = Math.abs(rider.latB - this.latB);
        var abs4 = Math.abs(rider.lonB - this.LonB);
...
<Text style={{ position: 'absolute', top: 150, borderWidth: 1, fontSize: '19px', fontWeight: 'bold'}}>{this.state.order_price}</Text>
