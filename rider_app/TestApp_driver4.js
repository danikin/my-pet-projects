...
import { StyleSheet, Dimensions, FlatList, Text, View, ScrollView, TextInput, Button, Image, ImageBackground, Icon } from 'react-native';
...
decorateRoute(current_point, route)
{
      ...
      if (n >= 2)
      {
        ...
        //shortest_angle_diff)
        {
          ...
          if (shortest_angle_diff < 0)
            arrow = "arrow-circle-o-right";
          else
            arrow = "arrow-circle-o-left";
         ...
         // console.log(`ANGLE_DIFF=${angle_diff}, SHORTEST_ANGLE_DIFF=${shortest_angle_diff}, prev_r=${prev_r}, cur_r=${cur_r}, R=${prev_r - cur_r}, arrow=${arrow},distance_to_turn=${distance_to_turn},
          ...
          return{
            ...
            next_turn_direction : (distance_to_turn < 50) ? "arrow-circle-up" : arrow
  ...
  return {
    ...
    next_turn_direction : "arrow-circle-up"           
  };
}
      ...
      this.assigned_order_id != -1)
  {
    ...
    if (this.state.assigned_order_status == "at_A")
    {
      ...
      let interval = setInterval(function() {
        let wait_time_so_far = Math.round(Date.now()/1000) - this.best_order.ts_driver_at_A;
        this.setState(
        {
          at_A_waiting_seconds : waiting_seconds,
          at_A_is_toll_waiting : is_toll_waiting,
          wait_time_minutes : Math.round(wait_time_so_far/60),
...
API_driver_accepted_toll_wait()
{
  this.ws.send(`{"event":"test_driver_accepted_toll_wait", "data":{"driver_id":${this.API_driver_id}}}`);
}
...
render()
{
  ...
  /*if (this.state.isLoading) {
    ...
    */
    ...
    if (this.state.page.startsWith("onborading") || this.state.page == "splash")
    {
      ...
      let render_onboarding_bottom = (
          ...
          onPress={() => this.setState({page: "best_order"})}>
              <Text style={{ fontSize: '25px', textTransform: 'uppercase', fontFamily: 'Roboto', fontWeight: 'bold'}}>Увидеть лучший заказ</Text>
            </TouchableOpacity>
            <View style={{flex: 1}}><Text></Text></View>
            <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
              <TouchableOpacity onPress={() => {this.setState({page: "onborading_about"})}}><FontAwesome5 name="chalkboard-teacher" size={36} color="black" /></TouchableOpacity>
              <TouchableOpacity><MaterialIcons name="feedback" size={36} color="black" /></TouchableOpacity>
              <TouchableOpacity onPress={() => {this.setState({page: "onborading_QnA"})}}><Entypo name="help" size={36} color="black" /></TouchableOpacity>
      ...
      // About the app
      if (this.state.page == "onborading_QnA")
      {
        return (
        <View style={styles.container}>
          <ScrollView style={{flex: 1, alignItems: 'center', justifyContent: 'center', position: 'absolute', left : 10, right : 10, top: 40}}>
            <View style={{flex: 1, flexDirection: 'row'}}>
              <Text style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}> Добро пожаловать в INSTANT </Text><MaterialCommunityIcons name="speedometer" size={24} color="black"/><Text style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}> TAXI</Text>
            </View>
              <Text style={{fontSize: '11px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>часто задаваемые вопросы</Text>
            <View style={{flex: 4}}><Text></Text></View>
            <View style={{flex: 20, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'left'}}>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>Вопрос: почему я с вашим приложеием заработаю больше?</Text></View>
              ...
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>Вопрос: если я заработаю больше, то пассажиры тоже будут платить больше? И если да, то откуда будет достаточный поток платящих пассажиров, чтобы загрузить меня заказами?</Text></View>
              ...
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>Вопрос: не является ли ваша низкая комисся заманухой? Не планируете ли вы ее повышать?</Text></View>
              ...
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>Вопрос: вы сможете гарантировать мне наличие заказов сразу после установки приложения?</Text></View>
    ...
    // About the app
    if (this.state.page == "onborading_about")
    {
      return (
        <View style={styles.container}>
          <ScrollView style={{flex: 1, alignItems: 'center', justifyContent: 'center', position: 'absolute', left : 10, right : 10, top: 40}}>
            <View style={{flex: 1, flexDirection: 'row'}}>
              <Text style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}> Чем INSTANT </Text><MaterialCommunityIcons name="speedometer" size={24} color="black"/><Text style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}> TAXI лучше?</Text>
            </View>
            <View style={{flex: 2}}><Text></Text></View>
            <View style={{flex: 2}}><Text></Text></View>
            <View style={{flex: 20, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'left'}}>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>1. Пассажиры платят только наличными или через Сбербанк.Онлайн, вы сразу получаете деньги</Text></View>
              <View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>2. Комиссия 3% или смена 150 руб. Вы сначала получаете деньги от пассажира, а потом платите комиссию или за смену через Киви, Элекснет или Сбербанк.Олайн.</Text></View>
              <View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>3. Вы всегда видите самый выгодный заказ, с автоматическим учетом в цене долгой подачи и возврата в место высокого спроса</Text></View><View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>4. Вы всегда видите цену, откуда, куда</Text></View><View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>5. Никаких штрафов за отказ, роботов, обязаловок</Text></View><View style={{flex: 1}}><Text></Text></View>
              <View><Text  style={{fontSize: '17px', color: 'black', fontWeight: 'bold', fontFamily: 'Roboto'}}>6. Свой тариф, фильтр районов, заказы по пути, фильтр по дистанции</Text></View>
            </View>
          </ScrollView>
          ...
          render_assigned_order_info = (
            <View style={{flex: 1, position: 'absolute', left : 10, right : 10, alignItems: "center"}}>
              ...
              this.setState({assigned_order_status: "ride_begins", next_turn_description_line1: "Едем с пассажиром", next_turn_description_line2: ""})
                ...
                <Text style={{fontFamaly: 'roboto', textTransform: 'uppercase', fontStyle: 'bold', fontSize: '25'}}> Пассажир сел, начинаю поездку </Text>
              </TouchableOpacity>

              <View style={{flex: 1, flexDirection: 'row'}}>
                <Text style={{fontFamily: 'roboto', textTransform: 'uppercase', fontWeight: 'bold', fontSize: '15'}}>Платное ожидание {Math.ceil(this.state.at_A_waiting_seconds/60)} минут</Text>
                <AntDesign name="checksquareo" size={36} color="black"/>
        ...
        else
        {
          render_assigned_order_info = (
            <View style={{flex: 1, position: 'absolute', left : 10, right : 10, alignItems: "center"}}>
              ...
              <View style={{flex: 1, flexDirection: 'row'}}>
                <Text style={{fontFamily: 'roboto', textTransform: 'uppercase', fontWeight: 'bold', fontSize: '15'}}>Ожидаю пассажира {this.state.wait_time_minutes/*Math.round(this.state.at_A_waiting_seconds/60)*/} мин, {/*Math.round(this.state.at_A_waiting_seconds) % 60*/this.state.wait_time_seconds} сек</Text>
              </View>
            </View>
          )
        }
  ...
  <View><FontAwesome5 name="flag-checkered" size={36} color="black" /></View>
            ...
            <MapView.Marker coordinate={this.state.next_turn_coordinate} ref={ref => { this.marker = ref}}>
              ...
              <MapView.Callout tooltip={true}>
                ...
                <FontAwesome name={this.state.next_turn_direction} size={36} color="black"/>
...
coordinates={this.state.coords2}
	...
	strokeWidth={10}/>
        ...
        <View style={{flex: 1, position: 'absolute', left : 10, right : 10, top: 30}}>
          ...
          <View style={{flex: 1}}><Text></Text></View>
          <View style={{flex: 1}}>
            <Text style={{color: 'black', fontSize: '11px', fontWeight: 'bold'}}>{(this.state.page == "ride_begins" || this.state.page == "payment")? this.state.addr_B : this.state.addr_A}</Text>
          </View>
          <View style={{flex: 1}}><Text></Text></View>
        ...
        <View style={{flex: 1, position: 'absolute', left : 10, right : 10, bottom: 20, alignItems: "center"}}>
          ...
          <View style={{flex: 0.4, flexDirection: 'row'}}>
            <TouchableOpacity><MaterialIcons name="feedback" size={36} color="black"/>
</TouchableOpacity>
            <TouchableOpacity><MaterialIcons name="chat" size={36} color="black"/>
</TouchableOpacity>
            <TouchableOpacity><AntDesign name="phone" size={36} color="black"/></TouchableOpacity>
            <TouchableOpacity><FontAwesome5 name="flag-checkered" size={36} color="black"/></TouchableOpacity>
            <TouchableOpacity><MaterialIcons name="watch-later" size={36} color="black"/></TouchableOpacity>
            <TouchableOpacity><MaterialIcons name="cancel" size={36} color="#d3d3d3"/></TouchableOpacity>
