                ...
                unsigned char *md5)
{
	...
	// Initialization
	uint32_t *X = reinterpret_cast<uint32_t*>(md5);
	enum {a0,b0,c0,d0};
	X[a0] = 0x67452301; // A
	X[b0] = 0xefcdab89; // B
	X[c0] = 0x98badcfe; // C
	...
	// Compute padding.
	const uint64_t OriginalMessageSize = size;
	...
	for (uint32_t chunk = 0; chunk < MessageSize; chunk += CHUNK_BYTESIZE) {
		const uint32_t *M = reinterpret_cast<const uint32_t*>(message + chunk);
